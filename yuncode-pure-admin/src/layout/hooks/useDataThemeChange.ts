...
export function useDataThemeChange() {
  ...
  function setLayoutThemeColor(
    ...
    // ⚠️ 空值检查：如果 $storage 或 layout 被清空（比如退出登录时），跳过
    if (!$storage || !$storage.layout) {
      console.warn("[setLayoutThemeColor] $storage.layout 不存在，跳过主题设置");
      return;
    }
  ...
  function dataThemeChange(overall?: string) {
    // ⚠️ 空值检查：如果 $storage 或 layout 被清空（比如退出登录时），跳过
    if (!$storage || !$storage.layout) {
      console.warn("[dataThemeChange] $storage.layout 不存在，跳过主题切换");
      return;
    }
